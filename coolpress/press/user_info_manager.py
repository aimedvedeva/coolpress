...
def get_github_data(github_username):
    ...
    if response.status_code == 200:
        ...
        css_selector_repos = 'div.UnderlineNav > nav > a:nth-child(2) > span'
        css_selector_stars = 'div.UnderlineNav > nav > a:nth-child(5) > span'
        try:
            repositories_info = soup.select_one(css_selector_repos)
            stars_info = soup.select_one(css_selector_stars)
            return int(repositories_info.text), int(stars_info.text)
